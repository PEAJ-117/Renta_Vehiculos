...
$(document).ready(function(){
    ...
    var table = $('#myTable').DataTable({
        "ajax":{
            "url":"../procesos/marcas/mostrar.php",
            "type":"GET"
        },
        "columns":[
            {
...
$(document).on('click', '.accionesTabla', function() {
    ...
    switch (val) {
                    ...
                    $.ajax({
                        ...
                        $('#btneditar').unbind().click(function(){
                            ...
                            if(noma.length != 0)
                                {
                             oka = {
						                "txtnom" : noma , "id_marcaa" : id
                                };
                            $.ajax({
                                method : "POST",
                                url : "../procesos/marcas/editar.php",
                                data : oka
                                }).done(function(msg) {
                                    $('#exampleModal2').modal('hide');
                                alertify.success("Marca Editada Correctamente!");
                                table.ajax.reload();
                                });                               
        ...
        case "Eliminar":
            ...
            alertify.confirm('Marca', '¿Esta seguro que desea eliminar esta marca?', function()
                {
                        $.ajax({
                                type:"POST",
                                url : "../procesos/marcas/eliminar.php",
                                data : "id="+id
                            }).done(function(msg) {
    ...
    $('#btnregistrar').click(function(){
       nom = $('#txtnombre').val();
        if(nom.length != 0 )
            ...
            $.ajax({
               type:'post',
                url:'../procesos/marcas/registrar.php',
                data:'txtnombre='+nom,
                success:function(r)
                {
